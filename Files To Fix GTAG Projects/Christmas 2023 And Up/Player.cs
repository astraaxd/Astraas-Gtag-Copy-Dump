	...
	public class Player : MonoBehaviour
	{
		...
		public void FixedUpdate()
		{
			...
			if (halloweenLevitationBonusStrength > 0f || halloweenLevitationStrength > 0f)
			{
				...
				{
					Mathf.InverseLerp(halloweenLevitateBonusOffAtYSpeed, halloweenLevitateBonusFullAtYSpeed, playerRigidBody.velocity.y);
					playerRigidBody.AddForce(Vector3.up * halloweenLevitationBonusStrength * Mathf.InverseLerp(halloweenLevitateBonusOffAtYSpeed, halloweenLevitateBonusFullAtYSpeed, playerRigidBody.velocity.y), ForceMode.Acceleration);
				}
			}
			...
			for (int i = 0; i < bodyOverlappingWaterVolumes.Count; i++)
			{
				...
				{
					float num3 = Vector3.Dot(result.surfacePoint - vector, result.surfaceNormal);
					if (num3 > num2)
					{
						num2 = num3;
					...
					if (current != null && num3 > 0f && !activeWaterCurrents.Contains(current))
					{
			...
			if (waterVolume != null)
			{
				...
				{
					float time = Mathf.Clamp(Vector3.Dot(velocity / scale, waterSurfaceForHead.surfaceNormal), swimmingParams.speedToBouyancyExtensionMinMax.x, swimmingParams.speedToBouyancyExtensionMinMax.y);
					float b = swimmingParams.speedToBouyancyExtension.Evaluate(time);
					buoyancyExtension = Mathf.Max(buoyancyExtension, b);
					float num4 = Mathf.InverseLerp(0f, swimmingParams.buoyancyFadeDist + buoyancyExtension, num2 / scale + buoyancyExtension);
					buoyancyExtension = Spring.DamperDecayExact(buoyancyExtension, swimmingParams.buoyancyExtensionDecayHalflife, fixedDeltaTime);
					num5 = num4;
				}
				else
				{
					num5 = Mathf.InverseLerp(0f, swimmingParams.buoyancyFadeDist, num2 / scale);
				}
				Vector3 vector2 = Physics.gravity * scale;
				Vector3 force = liquidProperties.buoyancy * -vector2 * num5;
			...
			if (swimmingParams.applyDiveSteering && !disableMovement && isDefaultScale)
			{
				...
				float num10 = swimmingParams.swimSpeedToMaxRedirectAngle.Evaluate(time2);
				float value2 = Mathf.Acos(Vector3.Dot(vector4, forward)) / MathF.PI * -2f + 1f;
				float value3 = Mathf.Acos(Vector3.Dot(vector4, forward2)) / MathF.PI * -2f + 1f;
				float num11 = Mathf.Clamp(value2, swimmingParams.palmFacingToRedirectAmountMinMax.x, swimmingParams.palmFacingToRedirectAmountMinMax.y);
				float num12 = Mathf.Clamp(value3, swimmingParams.palmFacingToRedirectAmountMinMax.x, swimmingParams.palmFacingToRedirectAmountMinMax.y);
				float a = ((!float.IsNaN(num11)) ? swimmingParams.palmFacingToRedirectAmount.Evaluate(num11) : 0f);
				float a2 = ((!float.IsNaN(num12)) ? swimmingParams.palmFacingToRedirectAmount.Evaluate(num12) : 0f);
				Vector3 vector6 = Vector3.ProjectOnPlane(vector4, right);
				Vector3 vector7 = Vector3.ProjectOnPlane(vector4, right);
				float num13 = Mathf.Min(vector6.magnitude, 1f);
				float num14 = Mathf.Min(vector7.magnitude, 1f);
				...
				num7 = Mathf.Min(a, Mathf.Min(a3, b3));
				float num15 = ((Vector3.Dot(vector4, forward) > 0f) ? (Mathf.Min(num7, b2) * num13) : 0f);
				num8 = Mathf.Min(a2, Mathf.Min(a4, b4));
				float num16 = ((Vector3.Dot(vector4, forward2) > 0f) ? (Mathf.Min(num8, b2) * num14) : 0f);
				if (swimmingParams.reduceDiveSteeringBelowVelocityPlane)
				{
					...
					float num17 = 1f - Mathf.InverseLerp(reduceDiveSteeringBelowPlaneFadeStartDist, reduceDiveSteeringBelowPlaneFadeEndDist, Mathf.Min(Mathf.Abs(f), Mathf.Abs(f3)));
					float num18 = 1f - Mathf.InverseLerp(reduceDiveSteeringBelowPlaneFadeStartDist, reduceDiveSteeringBelowPlaneFadeEndDist, Mathf.Min(Mathf.Abs(f2), Mathf.Abs(f4)));
					num15 *= num17;
					num16 *= num18;
				}
				float num19 = num16 + num15;
				Vector3 zero3 = Vector3.zero;
				if (swimmingParams.applyDiveSteering && num19 > num6)
				{
					zero3 = ((num15 * vector6 + num16 * vector7) / num19).normalized;
					zero3 = Vector3.Lerp(vector4, zero3, num19);
					vector5 = Vector3.RotateTowards(vector4, zero3, MathF.PI / 180f * num10 * fixedDeltaTime, 0f);
				}
				else
				{
					vector5 = vector4;
				}
				num9 = Mathf.Clamp01((num7 + num8) * 0.5f);
			}
			float num20 = Mathf.Clamp(Vector3.Dot(lhs, vector4), 0f, magnitude);
			float num21 = magnitude - num20;
			if (swimmingParams.applyDiveSwimVelocityConversion && !disableMovement && num9 > num6 && num20 < swimmingParams.diveMaxSwimVelocityConversion)
			{
				float num22 = Mathf.Min(swimmingParams.diveSwimVelocityConversionRate * fixedDeltaTime, num21) * num9;
				num20 += num22;
				num21 -= num22;
			}
			...
			float num23 = Spring.DamperDecayExact(num20 / scale, halflife, fixedDeltaTime) * scale;
			float num24 = Spring.DamperDecayExact(num21 / scale, halflife2, fixedDeltaTime) * scale;
			if (swimmingParams.applyDiveDampingMultiplier && !disableMovement)
			{
				float t = Mathf.Lerp(1f, swimmingParams.diveDampingMultiplier, num9);
				num23 = Mathf.Lerp(num20, num23, t);
				num24 = Mathf.Lerp(num21, num24, t);
				float time7 = Mathf.Clamp((1f - num7) * (num20 + num21), swimmingParams.nonDiveDampingHapticsAmountMinMax.x + num6, swimmingParams.nonDiveDampingHapticsAmountMinMax.y - num6);
				float time8 = Mathf.Clamp((1f - num8) * (num20 + num21), swimmingParams.nonDiveDampingHapticsAmountMinMax.x + num6, swimmingParams.nonDiveDampingHapticsAmountMinMax.y - num6);
			...
			swimmingVelocity = num23 * vector5 + zero * scale;
			playerRigidBody.velocity = swimmingVelocity + num24 * vector5;
		}
		...
		private void LateUpdate()
		{
			...
			Vector3 zero2 = Vector3.zero;
			if (swimmingParams.allowWaterSurfaceJumps && time - lastWaterSurfaceJumpTimeLeft > waterSurfaceJumpCooldown && CheckWaterSurfaceJump(lastLeftHandPosition, vector, leftControllerTransform.right, leftHandCenterVelocityTracker.GetAverageVelocity(worldSpace: false, 0.1f) * scale, swimmingParams, leftHandWaterVolume, leftHandWaterSurface, out var jumpVelocity))
			{
			...
			if (swimmingParams.allowWaterSurfaceJumps && time - lastWaterSurfaceJumpTimeRight > waterSurfaceJumpCooldown && CheckWaterSurfaceJump(lastRightHandPosition, vector2, -rightControllerTransform.right, rightHandCenterVelocityTracker.GetAverageVelocity(worldSpace: false, 0.1f) * scale, swimmingParams, rightHandWaterVolume, rightHandWaterSurface, out var jumpVelocity2))
			{
			...
			if ((leftHandColliding && !leftHandSlide) || (rightHandColliding && !rightHandSlide))
			{
		...
		public void BeginClimbing(GorillaClimbable climbable, GorillaHandClimber hand, GorillaClimbableRef climbableRef = null)
		{
			...
			NoncontrollableBroomstick component4;
		...
		public void EndClimbing(GorillaHandClimber hand, bool startingNewClimb, bool doDontReclimb = false)
		{
			...
			if ((bool)currentClimber)
			{
				...
				if (!startingNewClimb && (bool)currentClimbable)
				{
					...
					{
						playerRigidBody.velocity = bodyVelocityTracker.GetAverageVelocity(worldSpace: true) * 0.7f;
					}
					force = turnParent.transform.rotation * -gorillaVelocityTracker.GetAverageVelocity(worldSpace: false, 0.1f, doMagnitudeCheck: true) * scale;
