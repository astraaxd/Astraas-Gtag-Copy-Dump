...
public class VRRig : MonoBehaviour, IGorillaSerializeable, IPreDisable, IUserCosmeticsCallback, IGuidedRefTarget, IGuidedRefMonoBehaviour, IGuidedRefObject
{
	...
	GuidedRefTargetIdSO IGuidedRefTarget.GuidedRefTargetId
	{
		get
		{
			_ = guidedRefTargetInfo.targetId == null;
			return guidedRefTargetInfo.targetId;
		}
	}

	UnityEngine.Object IGuidedRefTarget.GuidedRefTargetObject => this;
	...
	public bool IsItemAllowed(string itemName)
	{
		...
		{
			if (PhotonNetwork.InRoom && GorillaGameManager.instance is GorillaBattleManager)
			{
				return true;
			}
			return false;
	...
	private void LateUpdate()
	{
		...
		else
		{
			if (voiceAudio != null)
			{
				...
				float num2 = (UsingHauntedRing ? HauntedVoicePitch : num);
				...
				if (isHaunted != playerWasHaunted)
				{
					if (isHaunted)
					{
						nonHauntedVolume = voiceAudio.volume;
						voiceAudio.volume = HauntedHearingVolume;
					}
					else
					{
						voiceAudio.volume = nonHauntedVolume;
					}
					playerWasHaunted = isHaunted;
				}
			}
		...
		if (creator != null)
		{
			...
			if (instance != null && instance.GetMaterialIfPlayerInGame(creator.ActorNumber, out var materialIndex))
			{
				tempMatIndex = materialIndex;
			}
			else
			{
	...
	public static void AttachLocalPlayerToPhotonView(PhotonView view, Vector3 offset, Vector3 velocity)
	{
		if (GorillaTagger.hasInstance && (bool)GorillaTagger.Instance.offlineVRRig)
		{
	...
	public void PlaySplashEffect(Vector3 splashPosition, Quaternion splashRotation, float splashScale, float boundingRadius, bool bigSplash, bool enteringWater, PhotonMessageInfo info)
	{
		...
		if (info.Sender == photonView.Owner && splashPosition.IsValid() && splashRotation.IsValid() && float.IsFinite(splashScale) && float.IsFinite(boundingRadius))
		{
			...
			if (time - 0.5f > num2)
			{
				...
				ObjectPools.instance.Instantiate(GorillaLocomotion.Player.Instance.waterParams.splashEffect, splashPosition, splashRotation, splashScale).GetComponent<WaterSplashEffect>().PlayEffect(bigSplash, enteringWater, splashScale);
			}
		}
		else
		{
	...
	private void OnDisable()
	{
		...
		try
		{
			fxSettings.callSettings[1].CallLimitSettings.Reset();
			fxSettings.callSettings[2].CallLimitSettings.Reset();
		}
		catch
		{
	...
	{
		if (cosmetics == rawCosmeticString && currentCosmeticTries < cosmeticRetries)
		{
